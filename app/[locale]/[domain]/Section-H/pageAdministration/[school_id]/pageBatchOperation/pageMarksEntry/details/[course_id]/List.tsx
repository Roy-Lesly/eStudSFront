...
const Select = ({ setSelectedType, searchParams, publishData }: { searchParams: { sem: "I" | "II" }, setSelectedType: any, publishData: EdgePublish }) => {
  ...
  return (
    <div className="bg-white border border-stroke dark:bg-boxdark dark:border-strokedark flex flex-col justify-center md:px-20 px-4 rounded-sm shadow-default">
      ...
      <div className="flex flex-col font-bold justify-between md:mt-4 md:py-4 mt-2 text-xl tracking-widest">
      ...
      {searchParams.sem == "I" && <div className='flex flex-col gap-10 items-center justify-center md:flex-row py-16'>
        {LinkOne.map((item: any) => (
          <button
            className={`${(item.pageType === "ca" && publishData?.node.portalCa) ? "flex" : (item.pageType === "exam" && publishData?.node.portalExam) ? "flex" : (item.pageType === "resit" && publishData?.node.portalResit) ? "flex" : checkPermission(token, ['result_management']) ? "flex" : "hidden"} bg-blue-900 font-semibold h-20 items-center justify-center px-10 rounded text-center text-white text-xl tracking-wide w-full`}
            key={item.id}
