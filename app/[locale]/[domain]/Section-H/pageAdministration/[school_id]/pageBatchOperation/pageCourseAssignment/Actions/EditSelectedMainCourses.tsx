...
const List = (
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (count === data.length) {
                router.push(`/${params.locale}/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageBatchOperation/pageCourseAssignment`);
    ...
    return (
        <div className="space-y-2 w-full text-black">
            ...
            >
                <span className="font-semibold col-span-2">{t("Course Name")}</span>
                <span className="hidden md:block">{t("Code")}</span>
                <span className="hidden md:block">{t("Credit")}</span>
                <span className="hidden md:block">{t("Hours")}</span>
                <span className="hidden md:block">{t("Type")}</span>
                <span className="hidden md:block">{t("Lecturer")}</span>
            </motion.div>
