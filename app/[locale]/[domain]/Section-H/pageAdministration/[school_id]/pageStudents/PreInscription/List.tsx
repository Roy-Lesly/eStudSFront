...
const List = ({ params, data, dataPending, searchParams }: { params: any; data: any, dataPending: any, searchParams: any }) => {
  ...
  const Columns: TableColumn<EdgePreInscription>[] = [
    ...
    { header: "Registration No", accessor: "node.registrationNumber", align: "left", responsiveHidden: true },
    { header: "Full Name", accessor: "node.fullName", align: "left" },
    { header: "Gender", accessor: "node.sex", align: "center", responsiveHidden: true },
    { header: "Address", accessor: "node.address", align: "center", responsiveHidden: true },
    ...
    {
      header: "View", align: "center", responsiveHidden: true,
      render: (item) => <button
          ...
          tabs={[
            {
              label: 'Pending', content: dataPending?.allPreinscriptions?.edges.length ?
                ...
                />
                :
                <ServerError type="notFound" item="Pending" />
            },
            {
              label: 'Admitted', content: data?.allPreinscriptions?.edges.length ?
                ...
                />
                :
                <ServerError type="notFound" item="Admitted" />
            },
            {
              label: 'Other Campus', content: data?.allPreinscriptions?.edges.length ?
                <MyTableComp
                  data={
                    dataPending?.allPreinscriptions?.edges.filter((i: EdgePreInscription) => i.node.campus.toUpperCase().replace("-", "_") !== data.allSchoolInfos.edges[0].node.campus.toUpperCase().replace("-", "_"))
                    .sort((a: EdgePreInscription, b: EdgePreInscription) => {                      
