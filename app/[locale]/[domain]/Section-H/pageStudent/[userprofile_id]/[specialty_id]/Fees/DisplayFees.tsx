...
const DisplayFees = ({ params, apiSchoolFees, apiTransactions }: { apiSchoolFees: EdgeSchoolFees, apiTransactions: EdgeTransactions[], params?: any }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center text-center">
        ...
        <p className="text-gray-600">{t("PageStudent.Fees.Academic Year")}: {apiSchoolFees.node.userprofile.specialty.academicYear}</p>
      </div>

      {/* Tuition Info */}
      <div className="flex flex-col gap-2 mt-4">
        <div className="flex justify-between">
          <span className="font-medium">{t("PageStudent.Fees.Level")}:</span>
          <span>{apiSchoolFees.node.userprofile.specialty.level.level}</span>
        </div>
        <div className="flex justify-between">
          <span className="font-medium">{t("PageStudent.Fees.Tuition")}:</span>
      ...
      <div className="bg-slate-200 font-bold my-4 py-1 rounded text-center">
        {t("PageStudent.Fees.All Transactions")}
      </div>
      ...
      <div className="mb-2 space-y-2">
        ...
        >
          <span>{t("PageStudent.Fees.No")}</span>
          <span className="truncate w-2/5">{t("PageStudent.Fees.Reason")}</span>
          <span>{t("PageStudent.Fees.Amount")}</span>
        </motion.div>
      </div>
      ...
      <div className="space-y-2">
        {apiTransactions && apiTransactions.length ? (
          ...
          <div className="text-center text-gray-500">{t("PageStudent.Fees.No Fee Transactions")}</div>
        )}
      </div>

      {/* Total Paid */}
      <div className="flex font-bold justify-between mt-6 text-lg">
        <span>{t("PageStudent.Fees.Total Paid")}:</span>
      ...
      <div className="bg-slate-100 flex items-center justify-between mt-4 p-3 rounded">
        <span className="font-medium">{t("PageStudent.Fees.Balance")}:</span>
  ...
  <span className="font-medium">{t("PageStudent.Fees.Request Moratorium")}:</span>
  ...
  {apiSchoolFees?.node?.platformPaid ? (
    ...
    <div className="flex items-center gap-2 text-red-500">
      <FaTimes size={25} />
      <span className="text-sm font-semibold">Account Inactive</span>
    </div>
  )}
</div>
      ...
      {/* Account Status & Action Button */}
      <div className="flex items-center justify-between mt-8">
        <span className="font-medium">{t("PageStudent.Fees.Account Status")}:</span>
        {apiSchoolFees.node.platformPaid ? (
          ...
          <FormModal
            ...
            buttonTitle={t("PageStudent.Fees.Activate")}
