'use client';
import { ConfigData, protocol, RootApi } from '@/utils/config';
...
const DisplayParentStudent = (
    { profile, source, role, p }:
        { profile: any, source: "H" | "S" | "P", role: "Student" | "parent", p: any }
) => {
    ...
    return (
        <>
            {data && <section className="my-6 px-3">

                <div className="bg-blue-950 h-[176px] px-5 py-2 rounded-lg w-full">
                    <div className="flex items-center justify-between">
                        <div className="text-white">
                            <span className="font-bold">{data?.customuser?.fullName}</span>
                            <div className="flex flex-col mt-2">
                                <div className="flex gap-2 justify-between">
                                    <span className="font-bold">
                                        {data?.specialty?.mainSpecialty.specialtyName}
                                        {data?.classroomsec?.level}
                                    </span>
                                </div>
                                ...
                                <div className="flex flex-row gap-2">
                                    <div className="flex gap-2 justify-between">
                                        <span className="font-bold">
                                            {data?.specialty?.academicYear}
                                            {data?.classroomsec?.academicYear}
                                        </span>
                                        |
                                        <span className="">
                                            {data?.specialty?.level?.level}
                                            {data?.classroomsec?.classType}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-row gap-2">
                                    <div className="flex gap-2 justify-between">
                                        {capitalizeFirstLetter(t("Matricle"))}: <span className="font-old italic tracking-widest">{data?.customuser?.matricle}</span>
                                    </div>
                                </div>
              ...
              />
                ...
                <div className='gap-3 grid grid-cols-2 grid-rows-3 mt-3 p-3 pb-16 rounded-[16px]'>
                    {menuList.map((list: any, index: number) => {
                        return (
                            <Link href={`/Section-S/pageParent/${p.userprofilesec_id}/${p.specialty_id}/${list.link}`} key={list.id} className='bg-white flex flex-col h-[86px] items-center justify-center rounded-[20px] shadow-3xl w-full'>
