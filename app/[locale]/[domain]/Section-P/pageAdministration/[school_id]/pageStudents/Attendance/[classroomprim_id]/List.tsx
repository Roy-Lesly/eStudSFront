...
import { EdgeAttendanceGeneralSecondary } from '@/utils/Domain/schemas/interfaceGraphqlSecondary';
...
const List = (
  { p, data, sp, classroom, attendance }:
    { p: any, data: EdgeUserProfile[], sp: any, classroom: EdgeClassRoomPrim, attendance: EdgeAttendanceGeneralSecondary }
) => {
  ...
  return (
    ...
    >
      <Breadcrumb
        department={`${t("")}`}
        subRoute={`${classroom?.node?.level} - ${classroom?.node?.cycle} - ${classroom?.node?.academicYear}`}
        pageName={`${t("Class Attendance")}`}
        mainLink={`${p.domain}/Section-P/pageAdministration/${p.school_id}/Settings/Students/${p.profile_id}`}
      />
