...
const SectionFooterThree = (
  { resultInfo, info, params }:
    { resultInfo: TypeProfileResults[], info: TypeTranscriptInfo, params: any }
) => {
  const [qrCodeDataUrl, setQrCodeDataUrl] = useState<string>("");

  const gpaRanges = resultInfo[0]?.gpaSystem === "GPA_5"
    ? [
      "4.5-5.00",
      "3.8-4.49",
      "3.1-3.79",
  ...
  const gpRanges = resultInfo[0]?.gpaSystem === "GPA_5"
    ? [
      "5.0",
      "4.4",
      "3.8",
      ...
      <View style={[styles.infoSection, { padding: 1 }]}>
        <View style={{ flexDirection: "column", justifyContent: "space-between" }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 4 }}>
            <Text style={{ fontWeight: "bold", textAlign: "left", gap: 2, width: "60%" }}>Level {resultInfo[0]?.level} - {resultInfo[0]?.yearGPA}</Text>
            <Text style={{ textAlign: "left", gap: 2, width: "100%" }}>The Director: {info?.schoolInfo?.schoolDirector}.</Text>
          </View>
          <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 4 }}>
            <Text style={{ fontWeight: "bold", textAlign: "left", gap: 2, width: "100%" }}>Level {resultInfo[1]?.level} - {resultInfo[1]?.yearGPA}</Text>
          </View>
          <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 4 }}>
            <Text style={{ fontWeight: "bold", textAlign: "left", gap: 2, width: "100%" }}>Level {resultInfo[2]?.level} - {resultInfo[2]?.yearGPA}</Text>
          </View>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={{ textAlign: "left", width: "60%" }}>
            Done in <Text style={styles.italic}>{info?.schoolInfo?.schoolTown}</Text>
          </Text>
          <Text style={{ textAlign: "left", width: "100%", gap: 10 }}>
...
export default SectionFooterThree;
