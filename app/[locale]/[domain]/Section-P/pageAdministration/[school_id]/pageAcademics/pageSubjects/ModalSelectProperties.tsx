...
const ModalSelectProperties = (
    ...
    const [getClassrooms, { data: classroomsData, loading: loadingClassrooms }] = useLazyQuery(GET_CLASSROOM_PRIM_QUERY);

    console.log(classroomsData?.allClassroomsPrim?.edges);

    useEffect(() => {
        const { academicYear, level, system } = formData;

        const allSelected = academicYear?.length && level?.length && system?.length > 0;
    ...
    }, [formData.academicYear, formData.level, formData.system, count]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        router.push(`/${params.locale}/${params.domain}/Section-P/pageAdministration/${params.school_id}/pageAcademics/pageSubjects/assign/?classId=${classroomsData?.allClassroomsPrim?.edges[0].node.id}`)
    };
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-2">
                    ...
                    <div className='flex flex-row gap-2 justify-between'>
                        <MyInputField
                            ...
                            options={(formData.system === "ENGLISH" ? apiLevels.slice(0, 9) : apiLevels.slice(9, 18)).map((item: string) => item)}
                    ...
                    {
                        (classroomsData?.allClassroomsPrim?.edges.length) ?
                            <motion.button
...
export const GET_CLASSROOM_PRIM_QUERY = gql`
    query 
        GetData(
            $academicYear: String!,
            $level: String!,
