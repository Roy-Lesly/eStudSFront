    ...
    return (
        <main className="bg-gradient-to-br flex from-blue-500 items-center justify-center min-h-screen pt-20 px-2 pb-20 to-purple-600">
                    ...
                    <div className="flex flex-col items-center mb-4">
                        <div className="bg-slate-200 border-1 border-slate-300 h-32 overflow-hidden rounded-lg w-32">
                            <div className="bg-blue-300 border-slate-300 flex h-32 items-center justify-center overflow-hidden p-1 rounded-lg w-32">
                                {data?.allUserProfiles.edges[0].node.customuser?.photo ? (
                                    <img
                                        // src={data?.allUserProfiles.edges[0].node.customuser.photo}
                                        src={`https://api${params.domain}.e-conneq.com/media/${data?.allUserProfiles.edges[0].node.customuser.photo}`}
                                        alt="Student Photo"
                ...
                {data?.allUserProfiles.edges.length === 1 ? (<>
                    <div className="flex flex-col gap-3 justify-center items-center text-center">
                        ...
                        <div className="flex flex-col">
                            ...
                            <p className="text-black font-bold text-2xl">{data?.allUserProfiles.edges[0].node.customuser?.matricle}</p>
                        </div>

                        <div className="flex w-full">
                            <div className="flex flex-col w-1/2">
                                <p className="font-medium text-slate-600">Date of Birth / Naissance:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser?.dob}</p>
                            </div>
                            <div className="flex flex-col w-1/2">
                                <p className="font-medium text-slate-600">Place of Birth / Naissance:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser?.pob}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex w-full">
                            <div className="flex flex-col w-1/2">
                                ...
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.nationality || "?"}</p>
                            </div>
                            <div className="flex flex-col w-1/2">
                                <p className="font-medium text-slate-600">Region Of Origin:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.regionOfOrigin || "?"}</p>
                            </div>
                        </div>
                        <div className="flex w-full">
                            <div className="flex flex-col w-1/2">
                                <p className="font-medium text-slate-600">Highest Certificate:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.highestCertificate || "?"}</p>
                            </div>
                            <div className="flex flex-col w-1/2">
                                <p className="font-medium text-slate-600">Year Obtained:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.yearObtained || "?"}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex w-full justify-center">
                            <div className="flex flex-col">
                                ...
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.parent || "?"}</p>
                            </div>
                        </div>
                        <div className="flex w-full">
                            <div className="flex flex-col w-full">
                                <p className="font-medium text-slate-600">Telephone:</p>
                                <p className="text-black font-bold">{data?.allUserProfiles.edges[0].node.customuser.parentTelephone || "?"}</p>
                            </div>
                        </div>
                    </div>
            ...
            <PasswordResetModal
                ...
                id={parseInt(decodeUrlID(data?.allUserProfiles.edges[0].node?.customuser?.id))}
            />
