import { EdgeSchoolFeesSec } from '@/Domain/schemas/interfaceGraphqlSecondary'
...
const Fees = ({ data }: { data: EdgeSchoolFeesSec }) => {
    ...
    >
      ...
      >
        <div className='flex gap-4 justify-center w-full'>
          ...
          <span className="font-bold text-red-600">{data.node.userprofilesec.classroom.paymentOne.toLocaleString()} F</span>
        </div>
        <div className='flex gap-4 justify-center w-full'>
          <span className="font-medium text-gray-700">2nd Installment</span>
          <span className="font-bold text-red-600">{data.node.userprofilesec.classroom.paymentTwo.toLocaleString()} F</span>
        </div>
        <div className='flex gap-4 justify-center w-full'>
          <span className="font-medium text-gray-700">3rd Installment</span>
          <span className="font-bold text-red-600">{data.node.userprofilesec.classroom.paymentThree.toLocaleString()} F</span>
        </div>
      </motion.div>
        ...
        {data.node.userprofilesec ? (
          ...
          >
            ...
            <span className="font-bold text-lg text-slate-950">
              {data.node.userprofilesec.classroom.tuition.toLocaleString()} F
            </span>
          </div>
        ) : null}
        {data.node ? (
          ...
          >
            ...
            <span className="font-bold text-lg text-slate-950">
              {(data.node.userprofilesec.classroom.tuition - data.node.balance).toLocaleString()} F
            </span>
          </div>
        ) : null}
        {data.node ? (
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="bg-white border border-gray-200 min-w-full">
            ...
            <tbody>
              {data.node.transactionssecSet.edges.map((transaction: any, index: number) => (
                <motion.tr
                  key={index}
                ...
                >
                  ...
                  <td className="px-4 py-2 text-gray-600">{transaction.node.createdAt}</td>
                </motion.tr>
              ))}
            </tbody>
          </table>
