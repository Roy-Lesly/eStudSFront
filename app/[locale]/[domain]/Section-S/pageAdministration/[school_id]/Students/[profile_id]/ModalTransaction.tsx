...
import { FaTimes, FaMoneyBillWave, FaUser, FaPhone, FaReceipt, FaEdit } from 'react-icons/fa';
...
const ModalTransaction = ({ setModalOpen, data }: { setModalOpen: any; data: any }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    const [createTransactionSec] = useMutation(CREATE_DATA)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createTransactionSec({ variables: newFormData });
      if (result.data.createTransactionSec.transactionsec.id) {
        alert(`Success creating:, ${result.data.createTransactionSec.transactionsec.reason}-${result.data.createTransactionSec.transactionsec.amount}`)
