...
import { TypeProfileResults, TypeTransSchoolInfo } from "@/utils/Domain/schemas/interfaceGraphqTranscript";



type GpaSystem = 'GPA_4' | 'GPA_5' | 'GPA_20';

const mapping: Record<GpaSystem, number> = {
  GPA_4: 4,
  GPA_5: 5,
  GPA_20: 20,
};
...
const SectionFooterOne = (
  { resultInfo, schoolInfo }:
  { resultInfo: TypeProfileResults, schoolInfo: TypeTransSchoolInfo}
) => {

  const gpaSystem = resultInfo?.gpaSystem as GpaSystem;

  const gpaRanges = resultInfo?.gpaSystem === "GPA_5"
      ? [
          "4.5-5.00",
          "3.8-4.49",
          "3.1-3.79",
  ...
  const gpRanges = resultInfo?.gpaSystem === "GPA_5"
      ? [
          "5.0",
          "4.4",
          "3.8",
      ...
      <View style={[styles.infoSection, { padding: 1 }]}>
        <View style={{ flexDirection: "column", justifyContent: "space-between" }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 4 }}>
            <Text style={{ textAlign: "left", gap: 2, width: "100%" }}>Attempted - {resultInfo?.yearCreditAttempted}</Text>
            <Text style={{ textAlign: "left", gap: 2, width: "100%" }}>Earned - {resultInfo?.yearCreditEarned}</Text>
          </View>
          <Text style={[styles.bold, {fontSize: 12, paddingTop: 1 }]}>
            GPA - {resultInfo?.yearGPA} / {mapping[gpaSystem]}
          </Text>
        </View>
        ...
        <View style={{ flexDirection: "row", justifyContent: "space-between"}}>
          <Text style={{ textAlign: "left", width: "100%"}}>{schoolInfo?.schoolDirector}</Text>
        </View>
        <View style={{ flexDirection: "row", justifyContent: "space-between"}}>
          <Text style={{ textAlign: "left", width: "100%"}}>Done in {schoolInfo?.schoolTown}</Text>
