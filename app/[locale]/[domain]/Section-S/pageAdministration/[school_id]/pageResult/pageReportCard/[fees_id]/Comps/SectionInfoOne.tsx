import { TypeTransStudentInfo } from "@/utils/Domain/schemas/interfaceGraphqTranscript";
import { View, Text, StyleSheet } from "@react-pdf/renderer";

const SectionInfoOne = (
  { studentInfo,  }: 
  ...
  <View style={[styles.row, { padding: 2, gap: 10 }]}>
    ...
    <View style={[styles.column]}>
      ...
      <View style={[styles.gridRow]}>
        ...
        <Text style={[styles.col2, styles.bold]}>{studentInfo?.domain}</Text>
      </View>

      {/* Field */}
      <View style={[styles.gridRow]}>
        <Text style={[styles.col2]}>Field:</Text>
        <Text style={[styles.col10, styles.bold]}>{studentInfo?.field}</Text>
      </View>

      {/* Specialty */}
      <View style={[styles.gridRow]}>
        <Text style={[styles.col2]}>Specialty:</Text>
        <Text style={[styles.col8, styles.bold]}>{studentInfo?.specialtyName}</Text>
      </View>

      {/* Academic Year & Level */}
      <View style={[styles.gridRow]}>
        <Text style={[styles.col2]}>Academic Year:</Text>
        <Text style={[styles.col2, styles.bold]}>{studentInfo?.academicYear}</Text>
        <Text style={[styles.col1]}>Level:</Text>
        <Text style={[styles.col1, styles.bold]}>{studentInfo?.level}</Text>
      </View>
    </View>
  </View>
</View>
