...
const CheckFormHigher = ({ data, p, sp }: { data: EdgePreInscription[]; p: any; sp: any }) => {
    ...
    const onSubmit = () => {
        ...
        router.push(`/${p.locale}/${p.domain}/pre-inscription/Check/higher/?${queryString}`);
    };
    ...
    useEffect(() => {
        if (clicked) {
            if (formData.telephone && data && data.length === 1) {
                ...
                const routeBase = `/${p.locale}/${p.domain}/pre-inscription/Check/higher`;
...
export default CheckFormHigher;
