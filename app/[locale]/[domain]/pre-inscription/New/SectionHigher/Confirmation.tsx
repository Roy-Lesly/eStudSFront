  ...
  return (
    ...
    >
      ...
      <section className="bg-white rounded-lg shadow p-4">
        ...
        {renderRow(
          t('Specialty 1st'),
          data.allMainSpecialties.edges.find(
            (item: EdgeMainSpecialty) =>
              decodeUrlID(item.node.id) === formData.classAssignment.specialty_one
          )?.node.specialtyName
        )}
        {renderRow(
          t('Specialty 2nd'),
          data.allMainSpecialties.edges.find(
            (item: EdgeMainSpecialty) =>
              decodeUrlID(item.node.id) === formData.classAssignment.specialty_two
          )?.node.specialtyName
        )}
        {renderRow(t('Program'), data.allPrograms.edges.find(
          (item: EdgeProgram) =>
            decodeUrlID(item.node.id) === formData.classAssignment.programId
        )?.node.name)}
        {renderRow(t('Level'), formData.classAssignment.level)}
