...
import { PRIMARY_LEVEL_CHOICES_ENGLISH, PRIMARY_LEVEL_CHOICES_FRENCH } from '@/utils/dataSource';
...
const PreFormPrimary = ({ data, source, params }: { params: any, source: "admin" | "student", data?: any }) => {
  ...
  const validateStep = (stepIndex: number): boolean => {
    switch (stepIndex) {
      ...
      case 2:
        const { academic_year, programId, level } = formData.classAssignment;
        return [academic_year, programId, level].every((field) => String(field).trim() !== '' && (Array.isArray(field) ? field.every(item => String(item).trim() !== '') : true));
  ...
  useEffect(() => {
    if (count === 0) {
      ...
      if (data && data.allProgramsprim?.edges) {
        const f = data.allProgramsprim.edges.map((item: EdgeProgram) => {
          return { "id": decodeUrlID(item.node.id), "name": `${item.node.name.replaceAll("_", ".")}` }
        })
        if (f) { setOptionsProgramsPrim(f) }
      }
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <MyInputField
                  ...
                  options={schoolSystem === "English Section" ? PRIMARY_LEVEL_CHOICES_ENGLISH : schoolSystem === "French Section" ? PRIMARY_LEVEL_CHOICES_FRENCH : []}
                />
