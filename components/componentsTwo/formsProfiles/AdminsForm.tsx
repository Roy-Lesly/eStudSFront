...
const AdminsForm = ({
  ...
  useEffect(() => {
    if (count == 0) {
      var getYears = async () => {
        var res = await getData(protocol + "api" + params.domain + AcademicYearUrl, {}, params.domain)
        var resDept = await getData(protocol + "api" + params.domain + GetDepartmentUrl, { nopage: true, name: "admin" }, params.domain)
        if (res && res.length) {
          if (data && data.year) { setApiYears(res.results.filter((item: string) => item != data.year)) }
          else { setApiYears(res.results) }
        };
        if (resDept && resDept.length) { setDept(resDept) }
        else { setDept([]) };
        setCount(1)
      }
      getYears()
  ...
  const onSubmit = handleSubmit((formVals) => {
    ...
    if (type === "create") {
      const call = async () => {
        ...
        if (t == "" && response && response.id) {
          router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/Admins?created="SUCCESSFULLY (${response.id}) !!!`);
          setOpen(false)
        }
        setClicked(false)
      }
    ...
    if (type === "update") {
      const call = async () => {
        const response = await ActionEdit(newData, data.id, SchemaCreateEditCustomUser, protocol + "api" + params.domain + CustomUserUrl, params.domain)
        if (response && response.id) {
          router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/Admins?updated="SUCCESSFULLY (${response.id}) !!!`);
          setOpen(false)
        }
        setClicked(false)
      }
    ...
    if (type === "delete") {
      ...
      const call = async () => {
        ...
        if (response && response.success) {
          router.push(`/${params.domain}/Section-H/pageAdministration/${params.school_id}/pageLecturers/Admins?deleted="DELETED (${data.id}) !!!`);
          setOpen(false)
        }
        setClicked(false)
      }
      ...
      {dept ?
        dept.length ?
          <form className="flex flex-col gap-4" onSubmit={onSubmit}>
            {type === "create" && <h1 className="font-semibold text-xl">Create Admin</h1>}
            {type === "update" && <h1 className="font-semibold text-xl">Update Admin</h1>}
            {type === "delete" && <h1 className="font-semibold text-xl">Delete Admin</h1>}
...
<InputField
          ...
          <div className="flex flex-col gap-4 h-full items-center justify-center w-full">
            <div>No Administration Department</div>
