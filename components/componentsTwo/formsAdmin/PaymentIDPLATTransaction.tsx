...
import { EdgeSchoolFees, EdgeTransactions, NodeSchoolHigherInfo } from '@/utils/Domain/schemas/interfaceGraphql';
import MyLoadingModal from '@/components/section-h/common/MyButtons/MyLoadingModal';


const PaymentIDPLATTransaction = (
  { source, setModalOpen, data, p, school, title }:
  { source: "admin" | "student" | "lecturer", setModalOpen: any; data: EdgeTransactions[], title: "single" | "multiple", p: any, school: NodeSchoolHigherInfo }
) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setClicked(true)
    const newData = {
      schoolfeesIds: data.map((sf: EdgeTransactions) => parseInt(decodeUrlID(sf.node?.schoolfees.id.toString())) ),
...
export default PaymentIDPLATTransaction;
