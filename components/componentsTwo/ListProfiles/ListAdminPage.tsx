...
const ListAdminsPage = async ({ params, searchParams, data }: { params: any, searchParams: any, data: GetCustomUserInter[] | any }) => {

  const apiYear = await getData(protocol + "api" + params.domain + AcademicYearUrl, { school: params.school_id }, params.domain)
    ...
    <div className="bg-white flex-1 m-2 mt-1 p-2 rounded-md">
      ...
      <div className="flex flex-col gap-4 items-center justify-between mb-2 md:flex-row md:gap-2 md:mb-4">
        <div className="flex gap-2 items-center w-full">
          <MyPageTitle title={`Admins ${data.length}`} />
          <TableSearch placeholder="By Name" searchString="full_name" />
          <div>
            {searchParams && Object.keys(searchParams).length ?
              <div className="flex flex-row gap-2 justify-end md:gap-2 w-full">
                <MessageModal
                  table="excel_profiles"
                  type="custom"
                  params={params} icon={<span className="flex gap-1 items-center justify-center p-1"><FaDownload size={16} /></span>}
                  data={data}
                  extra_data={{
                    searchParams: searchParams,
                    type: "profiles_admins",
                    export_title: `Admins`,
                    worksheet_name: "Admins List",
                    campus_id: params.school_id
                  }}
                  customClassName={`flex gap-2 bg-bluedash px-2 py-1 text-white font-medium capitalize gap-2 cursor-pointer rounded-full`}
                />
              </div> : null}
          </div>
